...
from umlsl_sim.rl.algorithms.rl_algorithm_registry import register_rl_algorithm
...
from umlsl_sim.rl.algorithms.sample_ppo_params import sample_ppo_params
...
class MaskablePPOAlgorithm(RLAlgorithm):
    ...
    :class:`umlsl_sim.control.safety.action_shield.ActionShield`, which asks the
    same `SafetyController` the safety-aware reward consults.

    Choosing this algorithm is what enables the shield:
    `RLRunner` reads `requires_action_masks` and calls
