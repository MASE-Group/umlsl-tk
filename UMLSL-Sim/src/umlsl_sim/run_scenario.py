...
Run as a module:
    ...
    python -m umlsl_sim.run_scenario                      # plain NPC simulation
    python -m umlsl_sim.run_scenario --scenario circuit --players 12
...
Two training mechanisms handle safety, and they are chosen with ``--algorithm``:
    ...
    python -m umlsl_sim.run_scenario --rl-mode TRAIN \
...
training, under ``rl_results/models/...``):
    ...
    python -m umlsl_sim.run_scenario --rl-mode LOAD_TRAINED_MODEL \
...
from umlsl_sim.scenario_io.loader import available_scenarios, load_scenario
...
def build_parser() -> argparse.ArgumentParser:
    modes = _rl_choices("umlsl_sim.reinforcement_learning.rl_modes", "RLMode")
    algorithms = _rl_choices(
        "umlsl_sim.reinforcement_learning.algorithms.rl_algorithm_types", "RLAlgorithmType"
    )
    observations = _rl_choices(
        "umlsl_sim.reinforcement_learning.gymnasium_env.observation_spaces.observation_model_types",
        "ObservationModelType",
    )
    rewards = _rl_choices(
        "umlsl_sim.reinforcement_learning.gymnasium_env.reward_types", "RewardType"
    )

    parser = argparse.ArgumentParser(
        prog="python -m umlsl_sim.run_scenario",
