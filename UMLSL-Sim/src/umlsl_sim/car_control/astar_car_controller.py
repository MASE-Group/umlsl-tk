...
from umlsl_sim.simulation.road_network.road_network import Intersection, LaneSegment, CrossingSegment, SegmentInfo, \
    right_direction, direction_sign
from umlsl_sim.constants import BUFFER, MAX_ACC, MAX_DEC, LANE_MAX_SPEED, CROSSING_MAX_SPEED, LEFT_LANE_CHANGE, \
...
class AstarCarController:
    ...
    def _violates_safety(self, current_segments: list[SegmentInfo],
        ...
        Order of checks:
          ...
          3. Rear-end rule over every lane segment of the projection: the
             projected reservation end must stay behind the worst-case next
             rear of every car ahead (see safety_checks.rear_end_violation).
        """
        ...
        if rear_end_violation(self.car, projected, self.reservation_management, cars_by_id):
            return True, False
